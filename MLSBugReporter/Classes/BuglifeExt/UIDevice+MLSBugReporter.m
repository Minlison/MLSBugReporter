...
@implementation UIDevice (MLSBRAdd)
...
static uint64_t yy_net_counter_get_by_type(yy_net_interface_counter *counter, MLSBRNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & MLSBRNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & MLSBRNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & MLSBRNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & MLSBRNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & MLSBRNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & MLSBRNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}
