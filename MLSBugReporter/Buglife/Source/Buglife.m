...
LIFEAttachmentType * const LIFEAttachmentTypeIdentifierCrashText   = @"crash.public.plain-text";
...
@property (nonatomic, strong) LIFEReportOwner *reportOwner;
@property (nonatomic, assign) BOOL debugMode;
@property (nonatomic, strong) LIFEReportWindow *reportWindow;
...
@property (nonatomic, strong) LIFEContainerWindow *containerWindow;
@property (nonatomic, assign) BOOL reportAlertOrWindowVisible;
@property (nonatomic, strong) LIFEDataProvider *dataProvider;
@property (nonatomic, strong) LIFEBugButtonWindow *bugButtonWindow;
@property (nonatomic, assign) UIInterfaceOrientation capturedOrientation; // the orientation of the device at time of screenshot capture
@property (nonatomic, assign) LIFEInvocationOptions lastUsedInovcationMethod;
@property (nonatomic, assign, getter=isScreenRecordingInvocationEnabled) BOOL screenRecordingInvocationEnabled;

@property (nonatomic, copy) NSString *userIdentifier;
@property (nonatomic, copy) NSString *userEmail;
@property (nonatomic, strong) LIFEAttachmentManager *attachmentManager;

@property (nonatomic, strong) LIFEInputField *userEmailField;
@property (nonatomic, strong) NSMutableDictionary<NSString *, LIFEAttribute *> *attributes;
// This should only be used when presenting the initial
// image editor
@property (nonatomic, strong, nullable) LIFEReportBuilder *reportBuilder;

/**
 These should be made public in an upcoming release.
 */
@property (nonatomic, copy, null_resettable) NSString *thankYouMessage;
@property (nonatomic, copy, null_resettable) NSString *titleForReportViewController;

// Legacy features, but some people might still want to use them.
// If they're dying for it, let them use it via private API.
@property (nonatomic, assign) BOOL hideUntilNextLaunchButtonEnabled;
@property (nonatomic, assign) BOOL useLegacyReporterUI;
...
{
    // 如果不允许监听截屏和录屏
    if (!(self.invocationOptions & LIFEInvocationOptionsScreenshot)) {
        return;
    }
    void(^HandleCompletion)(BOOL shouldHandle) = ^(BOOL shouldHandle) {
        if (shouldHandle && ((_invocationOptions & LIFEInvocationOptionsScreenshot) && !self.reportAlertOrWindowVisible)) {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(buglife:shouldHandleInvocation:completion:)]) {
        [self.delegate buglife:self shouldHandleInvocation:LIFEInvocationOptionsScreenshot completion:HandleCompletion];
    } else {
...
{
    // 如果不允许监听截屏和录屏
    if (!(self.invocationOptions & LIFEInvocationOptionsShake)) {
        return;
    }
    void(^HandleCompletion)(BOOL shouldHandle) = ^(BOOL shouldHandle) {
        if (shouldHandle && ((_invocationOptions & LIFEInvocationOptionsShake) && !self.reportAlertOrWindowVisible)) {
            UIImage *screenshot = [self _screenshot];
            [self _presentAlertControllerForInvocation:LIFEInvocationOptionsShake withScreenshot:screenshot];
        }
    };
    if (self.delegate && [self.delegate respondsToSelector:@selector(buglife:shouldHandleInvocation:completion:)]) {
        [self.delegate buglife:self shouldHandleInvocation:LIFEInvocationOptionsShake completion:HandleCompletion];
    } else {
        HandleCompletion(YES);
    }
}
...
- (void)_screenRecordingFinished
{
    ...
    void(^HandleCompletion)(BOOL shouldHandle) = ^(BOOL shouldHandle) {
        if (shouldHandle && (!self.reportAlertOrWindowVisible)) {
            [self _presentAlertControllerForInvocation:LIFEInvocationOptionsScreenRecordingFinished withScreenshot:nil];
        }
    };
    if (self.delegate && [self.delegate respondsToSelector:@selector(buglife:shouldHandleInvocation:completion:)]) {
        [self.delegate buglife:self shouldHandleInvocation:LIFEInvocationOptionsFloatingButton completion:HandleCompletion];
    } else {
        HandleCompletion(YES);
    }
}
...
{
    // 如果不允许监听截屏和录屏
    if (!(self.invocationOptions & LIFEInvocationOptionsFloatingButton)) {
        return;
    }
    void(^HandleCompletion)(BOOL shouldHandle) = ^(BOOL shouldHandle) {
        if (shouldHandle) {
...
- (void)_flushSettledAttacmentsAndAddToReportBuilder:(LIFEReportBuilder *)reportBuilder
{
    ...
    [_attachmentManager asyncFlushCandidateAttachmentsToQueue:dispatch_get_main_queue() completion:^(NSArray<LIFEReportAttachmentImpl *> * _Nullable settledAttachments) {
        __strong Buglife *strongSelf = weakSelf;
        if (strongSelf) {
            [reportBuilder addAttachments:settledAttachments];
        }
