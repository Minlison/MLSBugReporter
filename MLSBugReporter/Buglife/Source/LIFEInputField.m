...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    BOOL res = [self.attributeName isEqualToString:other.attributeName] &&
...
- (void)setTextFieldValue:(NSString *)textFieldValue {
    self.textViewCell.textView.text = textFieldValue;
    if (self.textViewCell.textView.lifeDelegate && [self.textViewCell.textView.lifeDelegate respondsToSelector:@selector(whatHappenedTextViewDidChange:)]) {
        [self.textViewCell.textView.lifeDelegate whatHappenedTextViewDidChange:self.textViewCell.textView];
    }
    self.textFieldCell.textField.text = textFieldValue;
    if (self.textFieldCell.delegate && [self.textFieldCell.delegate respondsToSelector:@selector(textFieldCellDidChange:)]) {
        [self.textFieldCell.delegate textFieldCellDidChange:self.textFieldCell];
    }
}
- (void)setTextFieldAttributeValue:(NSAttributedString *)attributeTextFieldValue {
    self.textFieldCell.textField.attributedText = attributeTextFieldValue;
    if (self.textViewCell.textView.lifeDelegate && [self.textViewCell.textView.lifeDelegate respondsToSelector:@selector(whatHappenedTextViewDidChange:)]) {
        [self.textViewCell.textView.lifeDelegate whatHappenedTextViewDidChange:self.textViewCell.textView];
    }
    self.textViewCell.text = attributeTextFieldValue.string;
    if (self.textFieldCell.delegate && [self.textFieldCell.delegate respondsToSelector:@selector(textFieldCellDidChange:)]) {
        [self.textFieldCell.delegate textFieldCellDidChange:self.textFieldCell];
    }
}
