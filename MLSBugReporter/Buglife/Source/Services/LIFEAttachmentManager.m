...
- (void)asyncFlushCandidateAttachmentsToQueue:(dispatch_queue_t)queue completion:(void (^)(NSArray<LIFEReportAttachmentImpl *> *settledAttachments))completion
{
    NSParameterAssert([NSThread isMainThread]);
    ...
    dispatch_async(_attachmentQueue, ^{
        __strong LIFEAttachmentManager *strongSelf = weakSelf;
        if (strongSelf) {
            NSArray *settledAttachments = strongSelf.candidateAttachments;
...
- (BOOL)_addAttachmentWithData:(NSData *)attachmentData type:(NSString *)attachmentType filename:(NSString *)filename error:(NSError * _Nullable * _Nullable)error requestsClosedHandler:(void (^)(void))requestsClosedHandler
{
    ...
    NSArray *validUTIs = @[LIFEAttachmentTypeIdentifierCrashText,LIFEAttachmentTypeIdentifierText, LIFEAttachmentTypeIdentifierJSON, LIFEAttachmentTypeIdentifierSqlite, LIFEAttachmentTypeIdentifierImage, LIFEAttachmentTypeIdentifierPNG, LIFEAttachmentTypeIdentifierJPEG];
