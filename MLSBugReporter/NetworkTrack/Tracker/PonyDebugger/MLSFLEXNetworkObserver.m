...
NSString *const kMLSFLEXNetworkObserverEnabledStateChangedNotification = @"kMLSFLEXNetworkObserverEnabledStateChangedNotification";
static NSString *const kMLSFLEXNetworkObserverEnabledDefaultsKey = @"com.flex.MLSFLEXNetworkObserver.enableOnLaunch";

typedef void (^NSURLSessionAsyncCompletion)(id fileURLOrData, NSURLResponse *response, NSError *error);

@interface MLSFLEXInternalRequestState : NSObject
...
@implementation MLSFLEXInternalRequestState

@end

@interface MLSFLEXNetworkObserver (NSURLConnectionHelpers)
...
@interface MLSFLEXNetworkObserver (NSURLSessionTaskHelpers)
...
@interface MLSFLEXNetworkObserver ()

@property (nonatomic, strong) NSMutableDictionary<NSString *, MLSFLEXInternalRequestState *> *requestStatesForRequestIDs;
@property (nonatomic, strong) dispatch_queue_t queue;

@end

@implementation MLSFLEXNetworkObserver
...
+ (void)setEnabled:(BOOL)enabled
{
    ...
    [[NSUserDefaults standardUserDefaults] setBool:enabled forKey:kMLSFLEXNetworkObserverEnabledDefaultsKey];
    ...
    if (previouslyEnabled != enabled) {
        [[NSNotificationCenter defaultCenter] postNotificationName:kMLSFLEXNetworkObserverEnabledStateChangedNotification object:self];
    }
}

+ (BOOL)isEnabled
{
    return [[[NSUserDefaults standardUserDefaults] objectForKey:kMLSFLEXNetworkObserverEnabledDefaultsKey] boolValue];
}
...
{
    static MLSFLEXNetworkObserver *sharedObserver = nil;
...
+ (void)sniffWithoutDuplicationForObject:(NSObject *)object selector:(SEL)selector sniffingBlock:(void (^)(void))sniffingBlock originalImplementationBlock:(void (^)(void))originalImplementationBlock
{
    ...
    // See https://github.com/Flipboard/MLSFLEX/issues/61 for an example.
    if (!object) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (numClasses > 0) {
            ...
            for (NSInteger classIndex = 0; classIndex < numClasses; ++classIndex) {
                ...
                if (class == [MLSFLEXNetworkObserver class]) {
                    continue;
                }
...
+ (void)injectIntoNSURLConnectionCancel
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = [MLSFLEXUtility swizzledSelectorForSelector:selector];
        Method originalCancel = class_getInstanceMethod(class, selector);

        void (^swizzleBlock)(NSURLConnection *) = ^(NSURLConnection *slf) {
            [[MLSFLEXNetworkObserver sharedObserver] connectionWillCancel:slf];
...
+ (void)injectIntoNSURLSessionTaskResume
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        void (^swizzleBlock)(NSURLSessionTask *) = ^(NSURLSessionTask *slf) {
            [[MLSFLEXNetworkObserver sharedObserver] URLSessionTaskWillResume:slf];
            ((void(*)(id, SEL))objc_msgSend)(slf, swizzledSelector);
        };
...
+ (void)injectIntoNSURLConnectionAsynchronousClassMethod
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        void (^asyncSwizzleBlock)(Class, NSURLRequest *, NSOperationQueue *, NSURLConnectionAsyncCompletion) = ^(Class slf, NSURLRequest *request, NSOperationQueue *queue, NSURLConnectionAsyncCompletion completion) {
            if ([MLSFLEXNetworkObserver isEnabled]) {
                NSString *requestID = [self nextRequestID];
                [[MLSFLEXNetworkRecorder defaultRecorder] recordRequestWillBeSentWithRequestID:requestID request:request redirectResponse:nil];
                NSString *mechanism = [self mechanismFromClassMethod:selector onClass:class];
                [[MLSFLEXNetworkRecorder defaultRecorder] recordMechanism:mechanism forRequestID:requestID];
                NSURLConnectionAsyncCompletion completionWrapper = ^(NSURLResponse *response, NSData *data, NSError *connectionError) {
                    [[MLSFLEXNetworkRecorder defaultRecorder] recordResponseReceivedWithRequestID:requestID response:response];
                    [[MLSFLEXNetworkRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:[data length]];
                    if (connectionError) {
                        [[MLSFLEXNetworkRecorder defaultRecorder] recordLoadingFailedWithRequestID:requestID error:connectionError];
                    } else {
                        [[MLSFLEXNetworkRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:data];
                    }
        ...
        [MLSFLEXUtility replaceImplementationOfKnownSelector:selector onClass:class withBlock:asyncSwizzleBlock swizzledSelector:swizzledSelector];
    });
}
    ...
    dispatch_once(&onceToken, ^{
        ...
        NSData *(^syncSwizzleBlock)(Class, NSURLRequest *, NSURLResponse **, NSError **) = ^NSData *(Class slf, NSURLRequest *request, NSURLResponse **response, NSError **error) {
            ...
            if ([MLSFLEXNetworkObserver isEnabled]) {
                ...
                [[MLSFLEXNetworkRecorder defaultRecorder] recordResponseReceivedWithRequestID:requestID response:temporaryResponse];
                [[MLSFLEXNetworkRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:[data length]];
                if (temporaryError) {
                    [[MLSFLEXNetworkRecorder defaultRecorder] recordLoadingFailedWithRequestID:requestID error:temporaryError];
                } else {
                    [[MLSFLEXNetworkRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:data];
                }
                if (error) {
        ...
        [MLSFLEXUtility replaceImplementationOfKnownSelector:selector onClass:class withBlock:syncSwizzleBlock swizzledSelector:swizzledSelector];
    });
}
...
+ (void)injectIntoNSURLSessionAsyncDataAndDownloadTaskMethods
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            if ([MLSFLEXUtility instanceRespondsButDoesNotImplementSelector:selector class:class]) {
            ...
            NSURLSessionTask *(^asyncDataOrDownloadSwizzleBlock)(Class, id, NSURLSessionAsyncCompletion) = ^NSURLSessionTask *(Class slf, id argument, NSURLSessionAsyncCompletion completion) {
                ...
                if ([MLSFLEXNetworkObserver isEnabled] && completion) {
                    NSString *requestID = [self nextRequestID];
                    NSString *mechanism = [self mechanismFromClassMethod:selector onClass:class];
            ...
            [MLSFLEXUtility replaceImplementationOfKnownSelector:selector onClass:class withBlock:asyncDataOrDownloadSwizzleBlock swizzledSelector:swizzledSelector];
        }
    });
}
...
+ (void)injectIntoNSURLSessionAsyncUploadTaskMethods
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            [MLSFLEXUtility replaceImplementationOfKnownSelector:selector onClass:class withBlock:asyncUploadTaskSwizzleBlock swizzledSelector:swizzledSelector];
        }
    });
}
...
+ (NSURLSessionAsyncCompletion)asyncCompletionWrapperForRequestID:(NSString *)requestID mechanism:(NSString *)mechanism completion:(NSURLSessionAsyncCompletion)completion
{
    NSURLSessionAsyncCompletion completionWrapper = ^(id fileURLOrData, NSURLResponse *response, NSError *error) {
        ...
        [[MLSFLEXNetworkRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:[data length]];
        if (error) {
            [[MLSFLEXNetworkRecorder defaultRecorder] recordLoadingFailedWithRequestID:requestID error:error];
        } else {
            [[MLSFLEXNetworkRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:data];
        }
...
+ (void)injectWillSendRequestIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionWillSendRequestBlock undefinedBlock = ^NSURLRequest *(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSURLRequest *request, NSURLResponse *response) {
        [[MLSFLEXNetworkObserver sharedObserver] connection:connection willSendRequest:request redirectResponse:response delegate:slf];
        return request;
    };
    ...
    [MLSFLEXUtility replaceImplementationOfSelector:selector withSelector:swizzledSelector forClass:cls withMethodDescription:methodDescription implementationBlock:implementationBlock undefinedBlock:undefinedBlock];
}
...
+ (void)injectDidReceiveResponseIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidReceiveResponseBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSURLResponse *response) {
        [[MLSFLEXNetworkObserver sharedObserver] connection:connection didReceiveResponse:response delegate:slf];
    };
...
+ (void)injectDidReceiveDataIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidReceiveDataBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSData *data) {
        [[MLSFLEXNetworkObserver sharedObserver] connection:connection didReceiveData:data delegate:slf];
    };
...
+ (void)injectDidFinishLoadingIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidFinishLoadingBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection) {
        [[MLSFLEXNetworkObserver sharedObserver] connectionDidFinishLoading:connection delegate:slf];
    };
...
+ (void)injectDidFailWithErrorIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidFailWithErrorBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSError *error) {
        [[MLSFLEXNetworkObserver sharedObserver] connection:connection didFailWithError:error delegate:slf];
    };
...
+ (void)injectTaskWillPerformHTTPRedirectionIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionWillPerformHTTPRedirectionBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionTask *task, NSHTTPURLResponse *response, NSURLRequest *newRequest, void(^completionHandler)(NSURLRequest *)) {
        [[MLSFLEXNetworkObserver sharedObserver] URLSession:session task:task willPerformHTTPRedirection:response newRequest:newRequest completionHandler:completionHandler delegate:slf];
...
+ (void)injectTaskDidReceiveDataIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDidReceiveDataBlock undefinedBlock = ^(id <NSURLSessionDataDelegate> slf, NSURLSession *session, NSURLSessionDataTask *dataTask, NSData *data) {
        [[MLSFLEXNetworkObserver sharedObserver] URLSession:session dataTask:dataTask didReceiveData:data delegate:slf];
    };
...
+ (void)injectDataTaskDidBecomeDownloadTaskIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDidBecomeDownloadTaskBlock undefinedBlock = ^(id <NSURLSessionDataDelegate> slf, NSURLSession *session, NSURLSessionDataTask *dataTask, NSURLSessionDownloadTask *downloadTask) {
        [[MLSFLEXNetworkObserver sharedObserver] URLSession:session dataTask:dataTask didBecomeDownloadTask:downloadTask delegate:slf];
    };
...
+ (void)injectTaskDidReceiveResponseIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDidReceiveResponseBlock undefinedBlock = ^(id <NSURLSessionDelegate> slf, NSURLSession *session, NSURLSessionDataTask *dataTask, NSURLResponse *response, void(^completionHandler)(NSURLSessionResponseDisposition disposition)) {
        [[MLSFLEXNetworkObserver sharedObserver] URLSession:session dataTask:dataTask didReceiveResponse:response completionHandler:completionHandler delegate:slf];
...
+ (void)injectTaskDidCompleteWithErrorIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionTaskDidCompleteWithErrorBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionTask *task, NSError *error) {
        [[MLSFLEXNetworkObserver sharedObserver] URLSession:session task:task didCompleteWithError:error delegate:slf];
    };
...
+ (void)injectDownloadTaskDidFinishDownloadingIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDownloadTaskDidFinishDownloadingBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionDownloadTask *task, NSURL *location) {
        ...
        [[MLSFLEXNetworkObserver sharedObserver] URLSession:session task:task didFinishDownloadingToURL:location data:data delegate:slf];
    };
...
+ (void)injectDownloadTaskDidWriteDataIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDownloadTaskDidWriteDataBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionDownloadTask *task, int64_t bytesWritten, int64_t totalBytesWritten, int64_t totalBytesExpectedToWrite) {
        [[MLSFLEXNetworkObserver sharedObserver] URLSession:session downloadTask:task didWriteData:bytesWritten totalBytesWritten:totalBytesWritten totalBytesExpectedToWrite:totalBytesExpectedToWrite delegate:slf];
    };
...
static char const * const kMLSFLEXRequestIDKey = "kMLSFLEXRequestIDKey";

+ (NSString *)requestIDForConnectionOrTask:(id)connectionOrTask
{
    NSString *requestID = objc_getAssociatedObject(connectionOrTask, kMLSFLEXRequestIDKey);
    if (!requestID) {
...
{
    objc_setAssociatedObject(connectionOrTask, kMLSFLEXRequestIDKey, requestID, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (id)init
{
    ...
    if (self) {
        ...
        self.queue = dispatch_queue_create("com.flex.MLSFLEXNetworkObserver", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (MLSFLEXInternalRequestState *)requestStateForRequestID:(NSString *)requestID
{
    MLSFLEXInternalRequestState *requestState = self.requestStatesForRequestIDs[requestID];
    if (!requestState) {
        requestState = [[MLSFLEXInternalRequestState alloc] init];
...
@implementation MLSFLEXNetworkObserver (NSURLConnectionHelpers)

- (void)connection:(NSURLConnection *)connection willSendRequest:(NSURLRequest *)request redirectResponse:(NSURLResponse *)response delegate:(id<NSURLConnectionDelegate>)delegate
{
    [self performBlock:^{
        NSString *requestID = [[self class] requestIDForConnectionOrTask:connection];
        MLSFLEXInternalRequestState *requestState = [self requestStateForRequestID:requestID];
        requestState.request = request;
        [[MLSFLEXNetworkRecorder defaultRecorder] recordRequestWillBeSentWithRequestID:requestID request:request redirectResponse:response];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection delegate:(id<NSURLConnectionDelegate>)delegate
{
    [self performBlock:^{
        NSString *requestID = [[self class] requestIDForConnectionOrTask:connection];
        MLSFLEXInternalRequestState *requestState = [self requestStateForRequestID:requestID];
        [[MLSFLEXNetworkRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:requestState.dataAccumulator];
...
@implementation MLSFLEXNetworkObserver (NSURLSessionTaskHelpers)
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler delegate:(id<NSURLSessionDelegate>)delegate
{
    [self performBlock:^{
        ...
        [[MLSFLEXNetworkRecorder defaultRecorder] recordMechanism:requestMechanism forRequestID:requestID];
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didWriteData:(int64_t)bytesWritten totalBytesWritten:(int64_t)totalBytesWritten totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite delegate:(id<NSURLSessionDelegate>)delegate
{
    [self performBlock:^{
        ...
        if (!requestState.dataAccumulator) {
            ...
            [[MLSFLEXNetworkRecorder defaultRecorder] recordResponseReceivedWithRequestID:requestID response:downloadTask.response];

            NSString *requestMechanism = [NSString stringWithFormat:@"NSURLSessionDownloadTask (delegate: %@)", [delegate class]];
            [[MLSFLEXNetworkRecorder defaultRecorder] recordMechanism:requestMechanism forRequestID:requestID];
        }

        [[MLSFLEXNetworkRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:bytesWritten];
    }];
}
...
- (void)URLSessionTaskWillResume:(NSURLSessionTask *)task
{
    ...
    [self performBlock:^{
        ...
        if (!requestState.request) {
            ...
            [[MLSFLEXNetworkRecorder defaultRecorder] recordRequestWillBeSentWithRequestID:requestID request:task.currentRequest redirectResponse:nil];
        }
    }];
}
