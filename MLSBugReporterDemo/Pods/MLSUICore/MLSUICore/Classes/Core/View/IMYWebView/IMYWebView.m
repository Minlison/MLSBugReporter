...
/** WKWebView 会不断调用start方法，用此属性做判断 */
@property (assign, nonatomic, getter=isStartNotiDelegate) BOOL startNotiDelegate;
@property (assign, nonatomic, getter=isEndNotiDelegate) BOOL endNotiDelegate;
@property (assign, nonatomic, getter=isErrorNotiDelegate) BOOL errorNotiDelegate;
@end
...
- (void)_initMyself {
        self.startNotiDelegate = YES;
        self.endNotiDelegate = YES;
        self.errorNotiDelegate = YES;
...
- (void)setAllowsInlineMediaPlayback:(BOOL)allowsInlineMediaPlayback {
        ...
        {
                [[(WKWebView *)self.realWebView configuration] setAllowsInlineMediaPlayback:allowsInlineMediaPlayback];
        }
}
...
- (void)callback_webViewDidFinishLoad {
        if ([self.delegate respondsToSelector:@selector(webViewDidFinishLoad:)] && self.isEndNotiDelegate) {
                [self.delegate webViewDidFinishLoad:self];
                self.endNotiDelegate = NO;
        }
}
- (void)callback_webViewDidStartLoad {
        if ([self.delegate respondsToSelector:@selector(webViewDidStartLoad:)] && self.isStartNotiDelegate) {
                [self.delegate webViewDidStartLoad:self];
                self.startNotiDelegate = NO;
        }
}
- (void)callback_webViewDidFailLoadWithError:(NSError*)error {
        if ([self.delegate respondsToSelector:@selector(webView:didFailLoadWithError:)] && self.isErrorNotiDelegate) {
                [self.delegate webView:self didFailLoadWithError:error];
                self.errorNotiDelegate = NO;
        }
}
...
- (void)evaluateJavaScript:(NSString*)javaScriptString completionHandler:(void (^)(id res, NSError * error))completionHandler {
        if (_usingUIWebView) {
...
- (void)sizeToFit {
        ...
        [self.realWebView sizeToFit];
        UIScrollView *scrollView = [self.realWebView scrollView];
        if (CGSizeEqualToSize(CGSizeZero, scrollView.contentSize))
        {
                [self reload];
                return;
        }
        frame.size = [(UIView *)self.realWebView frame].size;
        self.frame = frame;
}
- (CGSize)sizeThatFits:(CGSize)size {
        return [self.realWebView sizeThatFits:size];
}
#pragma mark - 清理
- (void)dealloc {
        if (_usingUIWebView) {
